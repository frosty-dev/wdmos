...
bool Core::find_functions()
{
	...
	FIND_OR_DIE(ToString, "55 8B EC 6A FF 68 E0 D1 ?? ?? 64 A1 ?? ?? ?? ?? 50 83 EC ?? 53 56 57 A1 ?? ?? ?? ?? 33 C5 50 8D 45 ?? 64 A3 ?? ?? ?? ?? 8B 4D ??");
	...
	FIND_OR_DIE(DelDatum, "55 8B EC 8B 4D 08 3B 0D ?? ?? ?? ?? 73 55 A1 ?? ?? ?? ?? 8B 04 88 85 C0 74 49 8B 50 ?? 81 FA 00 00 00 70");
	...
	int offset = *(int*)((char*)GetGlobalByName + 11);
	...
	char* datum_inc_function = (char*)Pocket::Sigscan::FindPattern(BYONDCORE, "55 8B EC 8B 4D 08 3B 0D ?? ?? ?? ?? 73 11 A1 ?? ?? ?? ?? 8B 04 88 85 C0 74 05 FF 40 18 5D C3 6A 21 51 E8 ?? ?? ?? ??");
	...
	FIND_OR_DIE(GetStringTableEntry, "55 89 E5 83 EC 18 8B 45 ?? 39 05 ?? ?? ?? ??");
	...
	FIND_OR_DIE(DelDatum, "55 89 E5 53 83 EC 44 8B 45 08 3B 05 ?? ?? ?? ?? 73 2C 8B 15 ?? ?? ?? ?? 8B 0C 82 85 C9 74 1F 8B 51 ?? 81 FA 00 00 00 70");
	...
	misc_entry_table = **(MiscEntry * ***)Pocket::Sigscan::FindPattern(BYONDCORE, "A1 40 ?? ?? ?? 8B 04 98 85 C0 74 ?? 89 04 24 E8 ?? ?? ?? ?? 8B 15 ?? ?? ?? ??", 1);
#endif
	return !failed;
}
